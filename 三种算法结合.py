...
from sklearn.cluster import KMeans  # 添加这个导入
...
# 定义投票分类器模型以优化K值
def optimize_k_value(features, max_clusters=10):
    param_grid = {'kmeans__n_clusters': range(2, max_clusters+1)}
...
def process_images_with_autoencoder_and_kmeans(image_folder, output_folder, n_clusters=5, layers=2, activation='leaky_relu'):
    ...
    kmeans = optimize_k_value(reduced_images, max_clusters=10)
    labels = kmeans.predict(reduced_images)
...
if __name__ == "__main__":
    input_folder = r"D:\shudeng\ProofingTool\聚类图象\自动优化alpha信噪比1234"
    output_folder = r"D:\shudeng\ProofingTool\聚类图象\聚类分析结果pca4"
