...
# 分配噪音类型的标签（扩展噪音类型）
def assign_noise_labels(n_clusters):
    # 定义多种噪音类型，按照簇的顺序（可以根据实际情况修改）
    noise_labels = {
        0: "低噪音",
        1: "高噪音",
        2: "周期性噪音",
    ...
    # 如果簇数大于定义的噪音类型数量，可以适当扩展标签
    return [noise_labels.get(i % len(noise_labels), f"类型 {i}") for i in range(n_clusters)]

# 可视化聚类结果，并标注噪音类型
def plot_image_clustering_results(reduced_images, labels, kmeans, output_folder):
    ...
    noise_types = assign_noise_labels(n_clusters)
    ...
    for i in range(n_clusters):
        ...
        # 进行标注
        plt.text(centroid[0], centroid[1], noise_types[i], fontsize=12, weight='bold', 
...
def process_images_with_autoencoder_and_kmeans(image_folder, output_folder, n_clusters=5):
    ...
    # 可视化并保存聚类结果
    plot_image_clustering_results(reduced_images, labels, kmeans, output_folder)

# 运行主程序
if __name__ == "__main__":
    input_folder = r"D:\shudeng\ProofingTool\数据\自动优化alpha信噪比123"
    output_folder = r"D:\shudeng\ProofingTool\数据\聚类分析结果pca2"
