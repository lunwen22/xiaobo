...
def plot_signal_comparison(original_signal, denoised_signal, file_name, output_folder):
    ...
    plt.title(f'多级自适应小波去噪信号 - {file_name}')
...
def process_folder(input_folder, output_folder):
    for file_name in os.listdir(input_folder):
        if file_name.endswith(".ini"):
            ...
            # 进行自适应多级小波去噪
            try:
                denoised_signal = adaptive_multilevel_wavelet_denoising(signal)
...
if __name__ == "__main__":
    ...
    output_folder = r"C:\Users\86136\Desktop\新建文件夹\jieguo_multilevel"  # 输出文件夹路径
