...
# 将处理后的信号保存为.ini文件
def save_signal_to_ini(signal, file_name, output_folder):
    if not os.path.exists(output_folder):
        os.makedirs(output_folder)

    output_path = os.path.join(output_folder, f'{file_name}.ini')
    
    with open(output_path, 'w') as file:
        for value in signal:
            file.write(f"{value}\n")
    
    print(f"已将处理后的信号保存到: {output_path}")
...
def process_folder(input_folder, output_folder):
    for file_name in os.listdir(input_folder):
        if file_name.endswith(".ini"):
            ...
            # 保存去噪后的信号为.ini文件
            save_signal_to_ini(denoised_signal, file_name.split('.')[0], output_folder)

# 主程序入口
if __name__ == "__main__":
    input_folder = r"D:\shudeng\波形图\log"
    output_folder = r"D:\shudeng\波形图\log\处理后的数据"
